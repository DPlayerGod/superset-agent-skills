...
- [x] Live preview: `create_chart`/`update_chart`/`create_dashboard` trả thêm `url`/`embed_url` dựng từ `SUPERSET_PUBLIC_URL`, gateway gom thành `previews`, panel nhúng iframe cùng-origin kèm 2 tab Preview/Explore.
- [x] Chatbox: nhiều đoạn chat lưu trong `localStorage` (mỗi đoạn giữ `session_id` cố định nên reload vẫn `--resume` đúng), nút phóng to, dropdown lịch sử.
- [x] Đo thời gian E2E mỗi lượt trong gateway (`timing.total_ms`, `first_event_ms`, `tool_calls`, `cli_duration_ms`), hiển thị badge trong panel.
- [x] Hạ tầng A/B baseline vs Agent Skills: `variant` trong request, `--plugin-dir` cho variant `skills`, nút "So sánh" chạy song song 2 variant.
- [ ] Viết nội dung skill thật vào `claude_gateway/skills_plugin/skills/` rồi đo lại baseline vs skills trên cùng bộ câu hỏi.
