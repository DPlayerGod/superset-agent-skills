...
gọi `mcp__superset-postgres__run_sql_readonly`, KHÔNG BAO GIỜ gọi `run_sql_readonly`
trần (tên trần sẽ lỗi "No such tool available", tốn một lượt gọi vô ích). Tên đầy đủ
của cả 10 tool dùng trong tài liệu này:
...
- `mcp__superset-postgres__run_sql_readonly`: chạy SQL SELECT/CTE để lấy số liệu thật
  (KHÔNG bịa số liệu).
...
- `mcp__superset-postgres__create_dataset`: đăng ký một bảng Postgres thành dataset
  trong Superset.
- `mcp__superset-postgres__create_chart`: tạo một chart dựa trên dataset đã có. LUÔN
...
- `mcp__superset-postgres__create_dashboard`: tạo dashboard MỚI và gắn các chart vào
  đó. LUÔN đi theo 2 bước, xem mục "Nhóm tạo chart/dashboard" bên dưới. Lưu ý thao tác
  gắn này THAY THẾ dashboard cũ của chart — chart đang nằm ở dashboard khác sẽ bị
  chuyển đi chứ không phải copy.
- `mcp__superset-postgres__add_charts_to_dashboard`: thêm chart vào một dashboard ĐÃ
  CÓ, giữ nguyên các dashboard hiện tại của chart. Dùng tool này khi người dùng muốn
  bổ sung chart vào dashboard sẵn có, thay vì tạo dashboard mới. Lưu ngay lập tức,
  không có bước preview riêng.

Nhóm tạo chart/dashboard — `mcp__superset-postgres__create_chart`,
`mcp__superset-postgres__create_dashboard`, cùng giao thức 2 bước như nhóm xoá bên
dưới:

1. Gọi tool KHÔNG kèm `confirm_token`. KHÔNG có gì được tạo trong Superset. Với
   `mcp__superset-postgres__create_chart` bạn nhận về một `embed_url` xem trước thật
   ...
   `mcp__superset-postgres__create_dashboard` bạn nhận về danh sách các chart sẽ được
